...
## The gamepiece's clickable area. This is used to detect when the gamepiece is clicked.
@onready var _click_area: = $"Animation/GFX/ClickArea" as Area2D
...
func _ready() -> void:
	...
	if not Engine.is_editor_hint():
		...
		# Snap the gamepiece to it's initial grid position. Note that the path's coordinates are decoupled from the gamepiece's in order to simplify
		# path creation (origin is the point of reference), so the follower needs to be initialized to the gamepiece's position.
