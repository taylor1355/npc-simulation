...
@onready var _collision_shape: = $CollisionArea/CollisionShape2D as CollisionShape2D
...
func _ready() -> void:
	if not Engine.is_editor_hint():
		...
		# Collisions will find the CollisionArea node as the collider. We'll point its owner reference to
		# the gamepiece itself to allow easily identify colliding gamepieces.
		$CollisionArea.owner = gamepiece
